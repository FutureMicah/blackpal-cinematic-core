...
-- Improve claim_contest_prize: validate that the contest period has ended,
-- return structured errors, and prevent claiming during active period.
...
BEGIN
  ...
  -- Parse period of form 'W{week}-{year}-{month}' to compute its end (next Monday 00:00 UTC).
  BEGIN
  ...
  IF now() < v_period_end THEN
    RETURN jsonb_build_object(
      'success', false,
      'code', 'contest_active',
      'error', 'Contest still active — claims open after period ends',
  ...
  RETURN jsonb_build_object(
    ...
    'tier', v_tier,
    'amount', v_amount,
    'pnl', COALESCE(v_pnl, 0),
    'period', p_contest_period,
    'claimed_at', now()
  );
END;
$function$;
